...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `tbl_apt_rizal` WHERE CONCAT(`no`, `firstname`, `lastname`, `email`,`contact`,`address`,`type_visa`,`date`,`time`,`type_apt`) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);
    
}
 else {
    //change table
    $query = "SELECT * FROM `tbl_apt_rizal`";
    $search_result = filterTable($query);
}
...
<html>
    ...
    <div class="header">
        <h1>Rizal - Appointments</h1>
    </div>

        <!-- change action -->
        <form action="rizal_dbd.php" method="post">
            <div class="search">
