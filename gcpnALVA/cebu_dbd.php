...
if(isset($_POST['search']))
{
    ...
    //CHANGE TABLE
    $query = "SELECT * FROM `tbl_apt_cebu` WHERE CONCAT(`no`, `firstname`, `lastname`, `email`,`contact`,`address`,`type_visa`,`date`,`time`,`type_apt`) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);
    
}
 else {
    //change table
    $query = "SELECT * FROM `tbl_apt_cebu`";
    $search_result = filterTable($query);
}
...
<html>
    ...
    <div class="header">
        <!-- change h1 -->
        <h1>Cebu - Appointments</h1>
    </div>

        <!-- change action -->
        <form action="cebu_dbd.php" method="post">
            <div class="search">
