...
//database connection
    ...
    if(!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['email']) && !empty($_POST['contact']) && !empty($_POST['address']) && !empty($_POST['type_visa']) && !empty($_POST['date']) && !empty($_POST['time']) && !empty($_POST['type_apt'] && !empty($_POST['cbox']))){   
        ...
        // change table
        $query = "insert into tbl_apt_makati(firstname,lastname,email,contact,address,type_visa,date,time,type_apt) values('$firstname','$lastname','$email','$contact','$address','$type_visa','$date','$time','$type_apt')";
...
<head>
    ...
    <!-- change title -->
    <title>Makati-Appointment</title>
</head>
<body>
    <div class="container">
        <!-- change form action -->
        <form action="makati_apt.php" method="post"> 
        <div class="header">
            <!-- change H2 -->
            <h2>MAKATI - Appointment</h2>
            <hr class="hr">
        </div>
