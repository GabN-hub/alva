<?php
    ...
    if(!empty($_POST['save']))
    {
        ...
        $ceb_e = 'alvacebu2010@gmail.com'; 
        $ceb_p = 'alvaCEBU';
        // $count = mysqli_num_rows($result);
        if($Email == $ceb_e && $Password == $ceb_p) {
            ?>
            <script>
                // change alert
                alert("Welcome! ALVA-Cebu!");
            </script>
            <?php
                // change url
                header("refresh:0; url=cebu_dbd.php"); 
        } else {
            ?>
            <script>
...
<head>
  ...
  <!-- change title -->
  <title>Cebu Confirmation</title>
</head>
<body>
  <div class="container">
    <!-- change action -->
    <form method ="POST" action="cebu_conf.php">
    <div class="header">
        <!-- change span -->
        <h1>Welcome! <span>ALVA-CEBU</span></h1>
        <hr class="hr">
    </div> 
    <div class="header">
